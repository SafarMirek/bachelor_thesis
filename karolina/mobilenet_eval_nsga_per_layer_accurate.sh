#!/usr/bin/env bash
#PBS -q qgpu_exp
#PBS -N evalNSGAPerLayerAssymetricAccurate
#PBS -l select=1:ngpus=1,walltime=00:05:00
#PBS -A OPEN-20-37

RUN=mobilenet_025_qat_12_no_act_approx_per_layer_asymmetric_24pch/run.00015.json.gz
...
python3 nsga_evaluate.py --multigpu --run nsga_runs/${RUN} --learning-rate 0.0025 --batch-size 64 --epochs 50 --bn-freeze 40 --act-quant-wait 20 --logs-dir-pattern logs/mobilenet/mobilenet_025_qat_12_no_act_approx_per_layer_asymmetric_24pch_24h_accurate/%s --checkpoints-dir-pattern checkpoints/mobilenet/mobilenet_025_qat_12_no_act_approx_per_layer_asymmetric_24pch_24h_accurate/%s
