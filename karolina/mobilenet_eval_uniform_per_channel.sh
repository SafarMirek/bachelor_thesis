#!/usr/bin/env bash
#PBS -q qgpu
#PBS -N evalUniformPerChannelSymetric
#PBS -l select=1:ngpus=7,walltime=5:00:00
#PBS -A OPEN-20-37

RUN=mobilenet_025_qat_12_no_act_per_channel_symmetric_24pch/run.00000.json.gz
...
python3 nsga_evaluate.py --multigpu --run nsga_runs/${RUN} --all --per-channel --symmetric --learning-rate 0.0025 --batch-size 64 --approx --epochs 50 --bn-freeze 40 --act-quant-wait 20 --logs-dir-pattern logs/mobilenet/mobilenet_025_uniform_per_channel_symmetric/%s --checkpoints-dir-pattern checkpoints/mobilenet/mobilenet_025_uniform_per_channel_symmetric/%s
