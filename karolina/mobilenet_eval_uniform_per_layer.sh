#!/usr/bin/env bash
#PBS -q qgpu
#PBS -N evalUniformPerLayerAssymetric
#PBS -l select=1:ngpus=1,walltime=8:00:00
#PBS -A OPEN-20-37
...
python3 nsga_evaluate.py --run nsga_runs/${RUN} --all --learning-rate 0.0025 --batch-size 64 --approx --epochs 50 --bn-freeze 40 --act-quant-wait 20 --logs-dir-pattern logs/mobilenet/mobilenet_025_uniform_per_layer_assymetric/%s --checkpoints-dir-pattern checkpoints/mobilenet/mobilenet_025_uniform_per_layer_assymetric/%s
