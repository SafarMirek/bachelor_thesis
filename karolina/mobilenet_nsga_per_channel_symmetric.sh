#!/usr/bin/env bash
#PBS -q qgpu
#PBS -N qatNSGAPerChannel025
#PBS -l select=1:ngpus=8,walltime=24:00:00
#PBS -A OPEN-20-37

echo "Mobilenet QAT NSGA Per-Channel Approx with disabled act quantization"
...
python3 run_nsga.py --multigpu --learning-rate 0.0025 --batch-size 64 --approx --symmetric --per-channel --act-quant-wait 12 --qat-epochs 12 --generations 50 --logs-dir nsga_runs/mobilenet_025_qat_12_no_act_per_channel_symmetric_24pch --parent-size 24 --offspring-size 24 --cache-datasets #--previous-run nsga_runs/mobilenet_025_qat_12_no_act_per_channel_symmetric_24pch
